  ...
  <div class="layout">
    <div>
      <section class="page-header" :style="'background-image: linear-gradient(120deg, ' + backgroundColorLeft + ', ' + backgroundColorRight + ');color: ' + fontColor + ';'">
        ...
        <h1 class="project-name">{{ obj.blogTitle }}</h1>
        <h2 class="project-tagline">{{ obj.blogDescribe }}</h2>
        <a :href="'https://github.com/' + obj.githubUsername" class="btn" target="_blank">GitHub主页</a>
        <a :href="`https://github.com/${obj.githubUsername}/vblog`" class="btn" target="_blank" v-if="!mini">博客源码</a>
      </section>
          ...
          <el-row>
            ...
            <el-col :span="8" style="text-align: center; padding: 12px 0px 0px 10px">
              <el-row>
                ...
                <el-col :span="6" style="padding: 9px 0px 0px 10px; color: #909399; font-size: 13px"> {{ $util.formatTime(music.currentTime) }}/{{ $util.formatTime(music.maxTime) }} </el-col>
              </el-row>
            ...
            <el-col :span="2" style="text-align: center">
              ...
              <el-popover ref="bigAvatar" placement="top-start" :title="obj.githubUsername" width="200" trigger="hover">
          ...
          <el-col :span="6" style="padding-right: 10px">
            <el-card shadow="never">
              <el-menu :default-active="active" @select="onSelect">
                <el-menu-item v-for="item in constantRouterMap" v-if="item.meta && item.meta.type == 'user' && !item.meta.LoginRequired && (!mini || !item.meta.mini)" :key="item.path" :index="item.index">
          ...
          <el-row>
            ...
            <el-col :span="10">
              <div style="float: right">
                <a href="https://developer.github.com" target="_blank">GitHub-API</a>&emsp;&emsp; <a href="https://cn.vuejs.org/" target="_blank">Vue.js</a>&emsp;&emsp;
...
export default {
  ...
  computed: {
    obj() {
      return this.$static.allPersonal.edges[0].node;
    },
  },
  watch: {
  ...
  methods: {
    onSelect(index, item) {
      const i = this.constantRouterMap.findIndex((item) => item.index === index);
      this.$router.push(this.constantRouterMap[i].path);
    },
    selectTopbar(index) {
      //取消菜单选中状态
